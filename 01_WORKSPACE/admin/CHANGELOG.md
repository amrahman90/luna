...
- **Retro-skeptic on session-60 finding** (`findings.md ## skeptic — session 62 (retro)` appended): session-60's "view_rdr_product one-hop IMG-download link" over-claim was UNSOUND — a bounded HEAD probe shows `view_rdr_product` is ALSO an HTML viewer (302→200 via `data.lroc.im-ldi.com` with PDS session cookies; content-type text/html). The actual IMG byte acquisition is form/cookie/JS-driven and lives one further hop downstream. HIGH-OPEN-METADATA-CHAIN tier PRESERVED; two-HTML-hop chain was verified, IMG byte not reached. Demanded wording fixes appended to `VERIFICATION.md` L31/L85/L127 + `findings.md` retro block.
- **Verifier session-60 (NAC verification fetch) — FAIL** (sha256 + size + source-URL all match; only stale-discipline failures). Corrections applied by orchestrator: R3 §1 L16 + §7 L94 (HIGH-OPEN-CHAIN → HIGH-OPEN-METADATA-CHAIN; drop "one-hop IMG-download" over-claim; corrected to "two HTML hops deep; form/cookie/JS-driven, ≥1 further hop"); R3 §9 L113 ("31 sessions" → "58 sessions" — second instance of the typo, the first fixed session-59 only touched §1 L20); MANIFEST L69–70 (stale "Sandbox path is NOT gitignored" paragraph deleted; replaced with "Sandbox HTML gitignored at .gitignore line 85" + corrected chain wording); VERIFICATION.md L31/L85/L127 (chain wording corrected); VERIFICATION.md L44 + §8 (cost phrasing removed per rule 1; "Cost ledger" → "Resource ledger"; $0 phrasing deleted).
- **Verifier session-59 (R3 status report) — PASS-with-notes**: all 15 acceptance criteria independently re-derived and match (registry md5; 117/161/45 decomp; PU v1 0.9000/0.8182/0.8968; v5 run B 0.824/0.737/14/15/0.930; deposit 10/10; sidecar sha; PROVENANCE rows 47+48; suite 124/8; smoke pins; f1-pin byte-identity; verifier scripts 21/15/11; R1 untracked; R2 supersession intact). Two notes (now both fixed above): "31 sessions" typo in §9 L113 (genuine FAIL-grade wording bug); §1/§7 view_rdr_product wording (same as verifier-60).
- **Process-deviation log (FIVE entries now in the permission-gap family):** (a) commit `0721854` body path-rule (s58); (b) sessions 49–56 orchestrator-direct bookkeeping (s58); (c) paper-writer `plans/` write gap → cp workaround (s59); (d) geo-coder R3 §7 in-place Python write (s61); (e) this session — orchestrator surgical edits to R3 §1/§7/§9, MANIFEST L69–70, VERIFICATION.md L31/L44/L85/L127/§8 because no subagent role cleanly owns plans/ content edits or sandbox verification-record wording fixes. Documented for the family record.
- **Step 0a Obsidian-context-read gap acknowledged:** sessions 59–61 did NOT load the protocol skill at session start (only session 58 did); corrected going forward. Both skills reloaded this session.
- **Vault catch-up sessions 58–61** (4 atomic notes) — staged to `notes/2026-09-12_session62_vault_catchup_pending_apply.md` (orchestrator bash apply per session-58/space-in-path precedent); 00_HOME refresh also staged.
...
- **Retro-skeptic** (the sessions 51/52/57 findings blocks had entered without skeptic review — protocol gap closed; block committed this session at `notes/findings.md` `## skeptic — session 58 (retro)`):
  - Session-51 gray-band block **SOUND-with-wording**: counts reproduced exactly 37/31, mixed provenance decisively confirmed — identical point sets, NaN differ by exactly 31; grid-resolution alternative refuted. NEW disclosure: PROVENANCE row 48's registration consumed the finite-ghost npz copy as reference — clamped by coarse_search np.clip, 31/60.9M, negligible — row 48 annotated this session.
  - Session-52 block **UNSOUND framing corrected**: the 85,304 ">100 m" points decompose as **85,273 real swath-edge geometry in (100,1000]** (median 102.3 m — a 65×125 m scanner-frame site legitimately extends past 100 m) + the same 31 ghosts; "old loader dropped them all" was FALSE (old SENTINEL_ABS=1000 dropped only the 31); cave_1x's 11,777 are all ≤105.4 m with zero gray-band. Demanded rewording appended by skeptic in findings.md `## skeptic — session 58 (retro)`.
  - Session-57 is_rung_duplicate=0 **SOUND-with-wording** (117/161/45→21 reproduced; unique_key is root-primary not 3-dp string). NEW disclosure: 6 cross-DTM near-duplicate ACTIVE pairs (12 rows, below-floor, FRESHMELT/FRESHMELT1 ×5 + TYCHOPK/TYCHOPK04 ×1) unresolved by superseded_by — below-floor so outside FP accounting; disclosure wording in skeptic block.
- **Vault-hygiene catch-up** (32 session notes + 00_HOME refresh PREPARED, apply pending): full content for `sessions/session_26.md` … `session_57.md` (one note per session, format matching `sessions/session_25.md`: heading `# Session <NN> — <date>`, 3–6 bullet summary, tags line, wikilink to MOC Sessions & Ops) plus the 00_HOME edits staged in `notes/2026-09-12_session58_vault_catchup_pending_apply.md` — the archivist edit/write tools are permission-blocked on the space-containing vault path (known opencode.json gap, session-44 B7 precedent: sidecar + orchestrator bash apply). CHANGELOG numbering gotcha: two 2026-08-23 entries are labelled session 31 ("Rounds 7-10 housekeeping" in the main body; "complete all" phase 2 in the appended tail, already mirrored as `sessions/session_25.md`); session_31.md covers Rounds 7-10 with a cross-reference.
- **00_HOME.md light refresh** (in the pending-apply sidecar): last-touched note → 2026-09-12 (session 58); Paper 1 → v2.1 submission-ready, Paper 2 → v3.3-draft; added line "v2-plan autonomous queue exhausted (sessions 51–57: sentinel hardening, io_common, B-cluster closed); remaining: D3 (deferred), NAC EDR re-probes (blocked), user-gated items". Registry/gate lines already current — untouched.
- **Process deviations logged (no action)**: (a) commit `0721854` body contains an elided absolute path (`/home/frostflux/...`) violating the 01_WORKSPACE-prefix commit rule — logged, no history rewrite (disproportionate; rewrite remedy is reserved for cost-phrase violations); (b) sessions 49–56 CHANGELOG entries + commits were applied by the orchestrator directly rather than via archivist dispatch (established in-project precedent, outcome-verified — noting for the record).

## 2026-09-12 (execution session 57 — B1–B5 cluster tail: byte-identical quoted-CSV writer + schema validation + structured-flags sidecar + B5 anchors proof)

- **registry_io extensions** (`code/wp5_fusion/registry_io.py`; geo-coder; verifier PASS-with-notes → both fixes applied): `SCHEMA_COLUMNS` (15, ordered, cites header line 31); `RegistrySchemaError` + `validate_registry(strict)` — checks column names/order/count, ≠15-field raw rows, duplicate ACTIVE ids, tier ∉ {A,B,C,D}, status ∉ {ACTIVE,SUPERSEDED}, null evidence; returns summary 278/117/161 on the frozen file. `write_registry(df, path, comment_header)` re-emits via csv.writer QUOTE_MINIMAL with the raw-parse stash in `df.attrs` (numeric coercion is lossy for `0.0050`-style literals). **Round-trip proof: BYTE-IDENTICAL — load frozen → write tmp → md5 == `a60fb52152e33f37e9052434ad026a6e`** (verifier reproduced independently).
- **Structured-flags sidecar** (new `code/wp5_fusion/registry_flags.py` + ONE new data file `data/candidate_registry_flags.csv`; frozen registry untouched): columns candidate_id,is_active,unique_key,is_tp,is_fp,is_ring,is_funnel,is_rung_duplicate,classification_source; classification derived from unique_accounting's OWN rules (not re-derived heuristically). **Sums verified: 278 rows; 14 TP + 9 FP + 21 ring + 1 funnel = 45 above-floor partition; 161 inactive SUPERSEDED; 117 ACTIVE.** CLI: bare → help exit 0 (guard added at verifier note; `--out` required to emit — house style session 49). **Finding of record: `is_rung_duplicate` = 0 structurally** — all multi-rung copies already carry status=SUPERSEDED via `superseded_by`; corroborated by sensitivity n_groups=21 == n_unique_above_floor. Scope note added (verifier-demanded wording): the flag counts ACTIVE×ACTIVE key collisions; the papers' 278→117 / 45→21 counts are row-level and include the 161 SUPERSEDED copies — no contradiction.
- **B5 verdict — anchors resolve, proven executable** (new `tests/test_registry_anchors.py`): all **278/278** registry rows resolve to one of the 54 `pair_results` entries on (dtm, rung_m parsed via `parse_rung_cm`); evidence pointers 100% conformant; no stragglers. Per-candidate records live IN the registry (peaks = sag_amp_m/span_m/score; rung in the id suffix) with pair context (raster path via `source_score_tif`) joined by evidence — B5 satisfied by the built architecture, now with an executable proof.
...
- **Test strengthened** (same file): AST-based `test_consumer_imports_unchanged` replaced with real `importlib.util.spec_from_file_location` + `exec_module` import that asserts (a) `preflight_clouds.png` mtime + sha256 (`ea376b2f…`) unchanged across the import, (b) `main` and `quick_dtm` callable + `SITES` populated, (c) AST checks retained for the other two consumers (`register_cave`, `coarse_search`).
- **CLI byte-identical** (back-up + run + restore): pre-refactor deterministic sha `4ed3c058887f27ace706e44e47197a5cff6d9e9acc8ecc42ab4c152fc8d59e45` ≡ post-refactor sha — refactor is purely structural. Canonical PNG **restored** to `ea376b2f…` (PROVENANCE_INDEX row 47 match); git status empty under data/outputs/wp1_analog/. Suite **89 passed** (73 sentinel + 6 io_analog + 8 sentinel — no net change).
- **Symmetry confirmed**: `register_cave.py` (L73/L213), `make_void_mask.py` (L53/L280), `coarse_search.py` (L122/L157) all already have `def main()` + `__main__` guards — only `explore_indian_tunnel.py` lacked them, now closed.
- **Pre-existing known gap** (not introduced by this session): the canonical `preflight_clouds.png` PROVENANCE_INDEX row 47 pins the historical `ea376b…` from before session-52's `io_analog` raise; running the script today would produce the session-52 deterministic `4ed3c0…`. PROVENANCE_INDEX update is a separate session-52 follow-up; not touched here.

## 2026-09-12 (execution session 52 — io_analog LOW-10 mirror: provenance pre-flight cleared, gray-band magnitudes bigger than session-51 framing, C-class flag for import-side-effect)

- **io_analog LOW-10 mirror** (`code/wp1_analog/io_analog.py` + new `code/tests/test_io_analog_sentinel.py`; geo-coder; verifier PASS-with-notes → SAFE TO SHIP): `SENTINEL_ABS = 1000.0` → `SENTINEL_MAX_M = 1e6` + `WARN_MAX_M = 100.0` (same constant-naming convention as `convert_f32`); one `warnings.warn` per `load_xyz` if kept |xyz| > 100 m (names file, count, max value, cites "session 52 / LOW-10 mirror"); public API (`load_xyz`, `voxel_downsample`, `master_grid`) signatures unchanged; consumers (`register_cave`, `coarse_search`, `explore_indian_tunnel`) import as before. Tests 80 → **89 passed**; NaN drop behavior unchanged.
- **Provenance pre-flight (verifier-required)**: the 4 PROVENANCE_INDEX rows 46–49 outputs (`coarse_search.json` `201208d…`, `preflight_clouds.png` `ea376b…`, `registration_report.json` `55078e…`, `registration_validation.png` `6ea5ee…`, all canonical 2026-08-21) **byte-identical pre/post** the change. Geo-coder caught and reverted an import-side-effect incident where `explore_indian_tunnel.py`'s module-level savefig overwrote the canonical PNG mid-debug — restored via `git checkout`, sha256 re-verified. Frozen corpus stands.
- **Real-data gray-band magnitudes on the npz files are bigger than session-51's .f32-source framing**: IndianTunnel_NorthSurface_1x_0p5m.npz has **85,304** kept points with |xyz| > 100 m (max 620,616.4 m); IndianTunnel_cave_1x has **11,777** (max 105.4 m). OLD loader silently dropped them; NEW loader keeps them visible-but-flagged with a one-shot warning. Any future re-run of `coarse_search`/`register_cave`/`explore_indian_tunnel` after this fix will produce LOUD divergence (warning names file/count/max) rather than silent corruption.
- **C-class flag** (separate ticket, not blocking): `wp1_analog/explore_indian_tunnel.py` L27–61 runs `load_xyz` + `plt.savefig` at module-import (no `__main__` guard). The test's AST-import workaround papers over a latent landmine — any future test/script that imports this module overwrites the canonical PNG. Recommended future refactor: wrap body in `main()` + gate with `if __name__ == "__main__":`. Recorded in findings.md.

## 2026-09-12 (execution session 51 — Wong-2014 Wayback verification + C15/LOW-10 sentinel hardening with verified data-quality finding)

- **Wong 2014 attribution verified via Wayback** (skeptic session-50 pre-submission flag): the archived NASA Ames page's own attribution block reads verbatim "Uland Wong, Warren Whittaker, Heather Jones, Red Whittaker. NASA Planetary Pits and Caves Analog Dataset. December, 2014." — the Paper 1 v2.1 reference `Wong, U., Whittaker, W., Jones, H., & Whittaker, R. (2014)` is correct as-is. **Flag closed clean; no paper change.**
- **C15 remainder / Hermes LOW-10** (`code/wp1_lla/convert_f32.py` + new `code/tests/test_convert_f32_sentinel.py`; geo-coder ×2 rounds; verifier FAIL→PASS): xyz sentinel threshold literal `1e3` → `SENTINEL_MAX_M = 1e6` constant; `WARN_MAX_M = 100.0` per-read warning (names file, count, max, LOW-10 citation; sentinels never warn); `ATTR_SENTINEL_MAX = 1e3` named for symmetry (color/NIR behavior unchanged); docstring/comment corrected. **Verifier-caught finding**: the round-1 "dormant change" claim was FALSE — read-only recount confirmed **37 gray-band points** (|xyz| ∈ (1e3, 1e6], max ≈ 930,515.8 m) in `Kingsbowl_orig.f32` and **31** (max ≈ 620,616.4 m) in `Indian_NorthSurface_1x.f32`; old code NaNed them, new code intentionally keeps them visible-but-flagged (the >100 m warning surfaces them by design). Narrative corrected to "NOT dormant; regeneration of the frozen corpus is forbidden"; gray-band points in any future conversion require explicit review. Additive `LAST_READ_DIAGNOSTICS` (path/n_total/n_xyz_sentinel/n_gray_band/gray_band_max_abs_m; signature unchanged; callers grep-clean) + CLI summary keys. **Mixed-provenance npz pinned**: `IndianTunnel_surface/Indian_NorthSurface_1x_0p5m.npz` KEEPS the 31 finite gray points (pre-1e3 conversion) while the `lltb1/` copy NaNs them — recorded in findings.md (`## data-quality — session 51`); frozen corpus NOT regenerated; frozen results stand. Tests 73 → **80 passed**; smoke canary unchanged (F1 0.392/0/0.800, AUC 0.990); zero writes under `~/lunarvoid/data/`.
- **Verifier loose ends fixed** (orchestrator, 1-liners): `verify_phase0_2026-09-04.py` check 8 now accepts both "KNOWN INSTALL QUIRK" and the reworded "KNOWN QUIRK" marker (8/9 → 9/9; the requirements.txt quirk itself was already documented); `verify_v02_f32dir_and_filter.py` docstring documents the required `PYTHONPATH=01_WORKSPACE/code` invocation (was false-failing 14/17 with ModuleNotFoundError `_crs` when run bare).
- **Follow-up flagged (not scheduled)**: `code/wp1_analog/io_analog.py` has its own `SENTINEL_ABS = 1000.0` on a separate code path — same dormant-risk class; audit before any future analog addition.

## 2026-09-12 (execution session 50 — Paper 1 v2.0 → v2.1 submission QA: reference rot purge + method refs)

- **Paper 1 reference audit (mirror of the P2 v3.3 rigor)** (paper-writer; skeptic SOUND-with-objections → all addressed; verifier PASS-with-notes): **8 of 9 references needed fixes, all Crossref/arXiv/ADS-verified verbatim** — Blair 2017 author order (Melosh was wrongly 4th; real order Blair/Chappaz/Sood/Milbury/Bobet/Melosh); Carrer 2024 author list (Hiesinger + "Mini-RF team" removed; Castelletti/Bruzzone/Patterson confirmed); Mueller 2026 conflated title → real "High-resolution mapping of snow depth and snow thermal insulation…" *Arctic Science* 12(1), M.M. not R.; Reichenzeller 2026 conflated title → real VCI stem-detection title, 17(7) not 17(8); Theinat 2018 wrong venue → 2018 AIAA SPACE Forum; **Wagner & Robinson 2021 (never live-verified before) conflated title** → real "Occurrence and origin of lunar pits: observations from a new catalog"; Wong 2014 initials swapped (Whittaker W., Jones H.). Kelahan + Le Corre clean.
- **Population phrasing fix**: "~281 catalogued pits" → "~300" at 8 sites (main ×4, cover letter, highlights, outline, referee template) — 281 was the impact-melt subset (281+15+5); shapefile = 278; JGR-2022 "almost 300". Skeptic verified all 8 sites mean total population (none broke); abstract's L30 correctly re-splits ~280/~20.
- **Skeptic must-have applied**: three load-bearing METHOD references added (all previously uncited by name): Planchon & Darboux 2002 *Catena* 46(2):159–176 (epsilon fill — canonical is 2002, not 2001); Wang & Liu 2006 *IJGIS* 20(2):193–213 (breach fill); Garwood 1936 *Biometrika* 28(3/4):437–442 (Poisson-exact CI). In-text anchors at first-use sites (L87 ×2, L113); ref count 9 → 12 across package (template, cover letter, reviewers enumeration). Header v2.1 note.
- **Reviewer-conflict catch**: Bickel demoted from suggested reviewers — real conflict is co-authorship on **Kelahan 2026** (not Blair 2017 as first hypothesized; skeptic corrected); Mittelholz promoted (on none of the 12 refs); Bickel proximity disclosed. Refuted "82-of-278" bookkeeping purged from Hemmi entry.
- **Consistency**: TRANQPIT1 two-structure statement intact (main rounded 17/133 m; template exact 16.5/132.8; zero 43 m / ≤0.1 m leftovers); frozen numbers spot-checked (3.74 ×7, 24,062.96 ×8, 278/117/233/45, 2.08 [0.67, 4.85]); "to our knowledge" added §1.3; GA plan header → v2.1; referee template self-count refreshed (307 lines / 8,617 words).
- **Open flags (pre-submission)**: Wong 2014 attribution rests on a dead NASA page — verify via Wayback before submission (skeptic); ESSA-F1 cover-letter refinement values (89.1/95.6) consistent but were undeclared in the header note.

## 2026-09-12 (execution session 49 — Paper 2 v3.3 submission polish + eval CLI safety + NAC EDR re-probe)

- **Paper 2 v3.2 → v3.3-draft** (paper-writer ×2 passes; skeptic SOUND-with-objections all addressed; verifier PASS + delta re-pass PASS-with-notes): **bibliographic corrections, all resolver-verified verbatim** — Powell was unpublished-Icarus → published *JGR Planets* 128(2), e2022JE007532 (matrix was right); Williams #18 title was a conflated non-record → verbatim Diviner global-temperatures title, *Icarus* 283, 300–325; **Hurwitz was citing a nonexistent Icarus 225 paper** → *PSS* 79–80, 1–38; **"Pozzobon" ref resolved to an unrelated 2019 earthquake-pounding paper** → replaced with the verified Sauro, Pozzobon, Massironi et al. 2020 review (*ESR* 209, 103288) + substantive §2 anchor; **both Cushing LPSC abstract numbers were wrong** (#1164=McCollom, #1951=Toplis) → corrected to the PDS bundle 2015 (doi:10.17189/1519222) + AbSciCon 2017 LPI Contrib. 1965 #3708 MGC3 catalog; Besserer "2024" entry (uncited, record likely nonexistent) deleted; Elkins-Tanton "(if cited)" placeholder deleted. **Number fixes** (v1 baseline JSON + registry as ground truth): v1 PU P/R detransposed → 0.9000/0.8182, AUC 0.8969→0.8968; INGENIIPIT ring 24/23→21 in 4 places (independent recount: 34 = 21 ring + 13 r001, IRIDIUMPIT1 below-floor). **Skeptic-demanded wording applied verbatim**: first-claim hedged ("to our knowledge, the first framework in the lunar literature to make FP per 10⁴ km² a mandatory reporting metric…"); code-availability corrected to on-request + E3-tied (was a false present-tense "open at the LUNARVOID repository"). Frozen v5 numbers untouched (F1 0.824 at 15 sites); 18 refs, all cited; diff audit: every hunk declared. Refs = 18 (8 shared with Paper 1 + 10 new).
- **Eval CLI safety** (geo-coder; verifier PASS): `wp5_fusion/pu_learning_groupsplit.py` + `pu_learning_extended.py` — bare invocation now prints help + exits (previously started a ~20-min battery); `--run` gate, `--list-runs` (LODO 21-folds plan + live registry counts), `--version` (evidence-JSON sha). No numerics/paths changed; no battery executed; wp5_fusion outputs byte-identical; pytest 73.
- **NAC EDR S3 re-probe (read-only)**: all legacy hosts dead for the cycles-3–5 products (pds.lroc.im-ldi.com 302→404, pds.mcp.nasa.gov/data/lroc 403, pdsimage2 404); `pds-img-archive-prod.s3.amazonaws.com` bucket EXISTS but every object path + listing → 403 AccessDenied (not anonymously readable, path not discoverable read-only). **Queued 1-line S3 patch still blocked** — needs either a working public path example or authenticated listing; cycles 3–5 remain deferred.

## 2026-09-12 (execution session 48 — B10 roadmap sync + R2 status report + Paper 2 v3.2 cosmetics + prior-art matrix genre rows)

- **B10 roadmap sync sweep** (archivist; verifier PASS-with-notes; R1-commit portion remains user-gated): 19 boxes ticked in the Next-Level v2 plan with commit-evidence suffixes (A1–A5, A7, B7/B8/B11, B10, C3, C4/C11/E4, C6, C7, D1, D4, E3-prep). Deliberate non-ticks annotated: B1–B5 cluster (quoted-CSV writer, schema validation, structured flags, per-run summaries, per-candidate records — session-42 precedent), C-io_common, C5, C15-remainder, D3, and all user-gated items. ZEROCOST/Next_Tasks docs untouched (sampled open boxes all genuinely undelivered). **R2 status report created**: `plans/2026-09-12_R2_Status_Report.md` — 6 sections built from CHANGELOG truth (executive state, frozen-results table, infrastructure state, user-gated queue, open non-gated items, R1/R2 lineage note). R2 supersedes the untracked R1 draft as the live status doc.
- **Paper 2 v3.1 → v3.2-draft** (paper-writer + orchestrator; verifier-queued cosmetics only, no content change): references NEW-block alphabetized (Watson & Baldini #17, Williams #18) with exactly one in-text ripple (§2 "entry 18"→"17"); #18 indent + initials normalized; title 3-line H1 → single-line H1 (wording byte-identical); header date corrected to 2026-09-12 (the v3.2-draft had briefly carried 09-11, backwards of v3.1's 09-12); two stale "33-ref prior-art matrix" mentions updated to 37-ref. Powell-venue and Williams-2017-title bibliographic oddities left as-is (flagged, out of cosmetic scope).
- **prior_art_matrix +4 genre rows** (paper-writer): appended Kelahan 2026 (2608.09350), Prasad & Mazumder 2026 (2607.03644 Moonstone), Purohit 2025 (2510.24010 Mars-Bench), Grethen 2025 (2510.18172 StereoLunar), with D4 niche-separation verdicts per row; WatsonBaldini2024 + LeCorre2025 already present (rows 28, 9). `.md` twin updated: 37 refs, 10 priority-done. Matrix and paper now consistent (37).
- **Parallel-write note**: R2 was drafted while the P2/matrix work ran in-flight; two stale §5 parentheticals amended by orchestrator pre-commit (verifier-flagged).
