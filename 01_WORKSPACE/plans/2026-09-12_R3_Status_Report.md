...
- **NAC reclassified BLOCKED → MEDIUM-OPEN (s59) → HIGH-OPEN-METADATA-CHAIN (s60)** — session-49's "blocked" was a wrong-URL call; WMS RDR metadata chain verified-fetchable (RDR product page 23,154 B; `view_rdr` HTML 10,370 B at sha256 `bb2c525c1ad622bc185c4e7c13a3febcf99a4780d83c8e77a88dc3cabb53c571`; PDS dataset `LRO-L-LROC-5-RDR-V1.0` + `LROLRC_2001` bundle + `view_rdr_product` HTML detail page, two HTML hops deep; IMG byte acquisition is form/cookie/JS-driven, ≥1 further hop); IMG byte-signature verification deferred to user-gated pipeline.
...
- **HIGH-confidence: reclassified from BLOCKED to MEDIUM-OPEN (session 59) -> HIGH-OPEN-METADATA-CHAIN (session 60 verified-fetch, sha256 `bb2c525c1ad622bc185c4e7c13a3febcf99a4780d83c8e77a88dc3cabb53c571`)**; HIGH is strictly with respect to "WMS RDR metadata chain is fetchable across two HTML hops + exposes the metadata needed for the IMG fetch (PDS dataset `LRO-L-LROC-5-RDR-V1.0` + LROLRC bundle `LROLRC_2001` + `view_rdr_product` HTML detail page; IMG byte acquisition is form/cookie/JS-driven, ≥1 further hop)" — the IMG byte-signature verification remains deferred to the user-gated NAC fetch pipeline (§11(b)). Pending user call to start the pipeline. Full observation logged in `findings.md ## data-acquisition — session 60` (session 59 entry retained for provenance); skeptic retro corrections in `findings.md ## skeptic — session 62 (retro)`.
...
**$0 over 58 sessions** (sessions 1–58). All probes Tier-0 or HTTP-only. Tier-1 (rented CPU/GPU) untouched; Tier-2 (paid GPU) untouched. No user-approved cost trigger fired. Ledger at `01_WORKSPACE/admin/budget.md`.
