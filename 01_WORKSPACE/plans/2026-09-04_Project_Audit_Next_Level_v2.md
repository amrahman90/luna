...
- [x] B1-B5 registry repair cluster (quoted-CSV writer, schema validation, row fixes + dedupe, MANIFEST/provenance, METHODS.md) — B1 done (69216eb: 15 rows + 3 methods + 161 SUPERSEDED / 117 unique); B2/B3/B4 done (cdb8f08: MANIFEST outputs-tree + policy, METHODS.md 943 lines, PROVENANCE_INDEX 160 artifacts); remainder open: quoted-CSV writer + schema validation + structured flag columns, per-run summary files, B5 per-candidate records — cluster stays open (partial tick not possible; session-42 precedent); cluster CLOSED session 57 (7bb51fd): byte-identical quoted-CSV writer + strict schema validation in registry_io; structured flags as sha-frozen-safe SIDECASTE data/candidate_registry_flags.csv (14/9/21/1 partition verified); per-run summary convention codified (findings.md); B5 verdict = anchors resolve 278/278, executable proof tests/test_registry_anchors.py
