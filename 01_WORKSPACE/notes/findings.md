...
## data-quality — session 52 (2026-09-11): io_analog LOW-10 mirror — provenance pre-flight cleared; real-data gray-band magnitudes bigger than session-51 framing

- **Provenance pre-flight (verifier)**: the 4 PROVENANCE_INDEX rows 46–49 (`coarse_search.json` sha256 `201208d…`, `preflight_clouds.png` `ea376b…`, `registration_report.json` `55078e…`, `registration_validation.png` `6ea5ee…`, all status=canonical, 2026-08-21) are **byte-identical pre/post** the io_analog change. The geo-coder's intermediate savefig side-effect incident (`explore_indian_tunnel.py` overwrote `preflight_clouds.png` during a debug import) was fully reverted (`git checkout`); sha256 re-verified. Frozen corpus stands.
- **Real-data gray-band magnitudes on npz (post-grid) are much bigger than .f32 source:** IndianTunnel_NorthSurface_1x_0p5m.npz has **85,304** kept points with |xyz| > 100 m (max 620,616.4 m); IndianTunnel_cave_1x has **11,777** (max 105.4 m). Session-51's 31/37 figures were the .f32 raw-source counts; the 0.5m-grid npz exposes many more ghost points. The OLD loader silently dropped them all; the NEW 1e6-threshold loader keeps them visible-but-flagged, with a one-shot >100 m warning per `load_xyz` that names file + count + max value.
- **Provenance-safety verdict: SAFE TO SHIP.** Frozen outputs (rows 46–49) are pinned and untouched; this commit hardens only FUTURE loads. Any consumer re-run that diverges from the pinned sha256s will surface a LOUD session-52 warning with file/count/max, never silently. The 89-passed pytest suite (80+9 new) keeps the AST-import test work-around (`explore_indian_tunnel.py` runs savefig at module-import — see C-class flag below).
...
## skeptic — session 58 (retro): sessions 51/52/57 findings audit

Retroactive skeptic pass (protocol gap: these entries landed without review). All verification read-only (venv numpy/pandas; no writes, no regeneration).

**Session 51 — SOUND-with-wording.** Independently reproduced via fresh memmap pass: Kingsbowl gray=37 (max 930,515.8), NorthSurface gray=31 (max 620,616.4), sentinels 924/385 — exact match. Grid-resolution alternative REFUTED: both npz carry identical n=60,959,587 (== f32 point count) and identical (100,1000] population (85,273 each, same median 102.3/p99 110.6); NaN counts differ by exactly 31 (419 vs 388) = the gray band — mixed provenance is the only consistent explanation. Objection: PROVENANCE row 48 (`registration_report.json`) names `reference_npz = lltb1/IndianTunnel_surface/Indian_NorthSurface_1x_0p5m.npz` — the FINITE-ghost copy. So "all downstream frozen results stand" is true as policy, but the finite-ghost copy had a canonical consumer (rows 46–49). Materiality negligible-but-undisclosed: coarse_search clamps out-of-grid x/y to edge cells (np.clip, coarse_search.py L50–51), so the 31 ghosts were clamped, not excluded (31/60.9M; moving cloud cave_1x has zero gray points, max 105.4 m). DEMANDED WORDING: annotate rows 46–49 in PROVENANCE_INDEX — "reference npz is the pre-filter copy; 31 gray-band points clamped to grid edge by coarse_search; <1e-6 of points; no registration impact expected" — and add one line to the session-51 block.

**Session 52 — UNSOUND (framing; numbers correct).** Decomposition of the 85,304: **85,273 lie in (100, 1000]** (median 102.3 m, p99 110.6 m — real edge-of-swath geometry; 65×125 m site in scanner frame legitimately spans >100 m) and **31 in the gray band** — the SAME 31 as the .f32. cave_1x's 11,777 are ALL ≤105.4 m — **zero gray-band points**. The header "gray-band magnitudes … much bigger than session-51 framing" is a category error: it compares a >100 m WARNING-band count to a >1e3 m GRAY-band count. "The 0.5m-grid npz exposes many more ghost points" is false — identical point set to the f32. "The OLD loader silently dropped them all" is false: old `SENTINEL_ABS = 1000` (io_analog.py at ab62166^) NaNed only the 31; the 85,273 were kept silently (no warning existed — the real, milder hygiene point). Provenance sha pre-flight and C-class flag: no objection. DEMANDED WORDING: replace the second bullet with: "Warning-band (>100 m, ≤1e3) population is large — 85,273 NorthSurface (median 102.3 m; plausibly real swath-edge geometry) and 11,777 cave_1x (all ≤105.4 m) — while gray-band (>1e3) counts are unchanged from .f32 source (31; cave_1x: 0). Old loader (>1e3) NaNed only the 31 gray points; the 85,273 were kept without warning and remain kept (now warned) under the new loader."

**Session 55 convention — SKIP** (process hygiene; not scientifically load-bearing).
