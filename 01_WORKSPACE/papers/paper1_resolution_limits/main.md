...
**Version:** v2.1 (2026-09-11) — submission QA pass over v2.0. Fixes: seven reference records corrected to resolver-verified verbatim form (Blair author list; Carrer author list — Castelletti/Bruzzone replace the erroneous "Hiesinger / Mini-RF team"; Mueller title, initial, and issue; Reichenzeller title and issue 17(7); Theinat venue — 2018 AIAA SPACE Forum, not SciTech; Wagner & Robinson title — "Occurrence and origin of lunar pits: observations from a new catalog"; Wong attribution per the dataset page); "~281 catalogued pits" → "~300" (the cited LPSC 2021 abstract counts ~281 pits in impact melt plus 15 mare and 5 highland pits; the distributed PDS shapefile carries 278 entries); "to our knowledge" added to the §1.3 first-metrics claim. No frozen number, table, or claim of substance changed; method references added (Planchon–Darboux 2002; Wang & Liu 2006; Garwood 1936) per skeptic session 50.
...
**Catalogues and imagery-based detectors.** The Lunar Pit Atlas (Wagner and Robinson, 2021) is the primary morphometric compilation of catalogued lunar pits (~300 entries — ~280 in impact-melt deposits, ~20 in mare and highland terrain; positional accuracy ~30 m) and supplies the label set used by essentially all downstream machine-learning work. The most prominent recent entry is the ESSA detector of Le Corre et al. (2025), a Mask R-CNN trained on Atlas labels augmented with Martian HiRISE pits and synthetically implanted pits, which reports strong detection scores on NAC imagery. Two properties of that line of work motivate ours. First, its evaluation scores are detection metrics computed against the same catalogued pit population used in training, without per-claim void probabilities or false-positive-per-area reporting; at the lunar base rate (Section 1.1) such scores do not transfer into per-site confidence. Second, it is a two-dimensional imagery method: it finds pit-like *appearances*, not the roof-sag *topographic* signature of an intact tube whose interior is not exposed. We cite ESSA as the closest direct competitor, position our contribution as calibrated inference rather than detection, and never use its outputs as labels. Beyond supervised pit detection, unsupervised anomaly search over NAC images has recently recovered volcanic pits and collapsed lava tubes among its anomaly classes, alongside rockfalls, fresh craters, and irregular mare patches (Kelahan et al., 2026). That line, like ESSA, is a two-dimensional imagery method; the present work addresses the orthogonal question of what calibrated morphometric inference from orbital DTMs can claim, with explicit false-positive accounting per unit area.
...
2. **To our knowledge, the first per-rung detectability metrics for the LROC NAC GSD band** on surveyed ground truth, reported as F1/precision/recall with per-rung false-positive densities, and with the predict-all collapse of the threshold tuner reported as a finding rather than hidden.
...
- **Depression depth** — the sink-filled minus the observed surface, using the Planchon–Darboux epsilon fill (Planchon and Darboux, 2002). The fill algorithm is not interchangeable: breach-based fills of the Wang & Liu type (Wang and Liu, 2006) drain the NoData floors of shadowed pit interiors and return near-zero depth where the true floor is ~130 m down (quantified at a lunar pit in Section 4.4), whereas the epsilon fill recovers shadowed-floor depth.
...
- Otherwise it is a **false positive**, and the per-rung and aggregate rates are FP per 10⁴ km² with Poisson-exact (Garwood, 1936) 95% confidence intervals — appropriate for small FP counts — over the processed area.
...
Blair, D.M., Chappaz, L., Sood, R., et al. (2017). The structural stability of lunar lava tubes. *Icarus* 282, 47–55. doi:10.1016/j.icarus.2016.10.008.

Carrer, L., Pozzobon, R., Sauro, F., Castelletti, D., Patterson, G.W., & Bruzzone, L. (2024). Radar evidence of an accessible cave conduit on the Moon below the Mare Tranquillitatis pit. *Nature Astronomy* 8(9), 1119–1126. doi:10.1038/s41550-024-02302-y.

Garwood, F. (1936). Fiducial limits for the Poisson expansion. *Biometrika* 28(3/4), 437–442. doi:10.1093/biomet/28.3-4.437.

Kelahan, C., Angerhausen, D., Lesnikowski, A., & Bickel, V.T. (2026). A machine learning based search for lunar anomalies. *arXiv preprint* arXiv:2608.09350 [astro-ph.EP]. https://arxiv.org/abs/2608.09350
...
Mueller, M.M., et al. (2026). High-resolution mapping of snow depth and snow thermal insulation using low-cost UAVs in complex Arctic urban terrains. *Arctic Science* 12(1), 1–23. doi:10.1139/as-2025-0062.

Planchon, O., & Darboux, F. (2002). A fast, simple and versatile algorithm to fill the depressions of digital elevation models. *Catena* 46(2), 159–176. doi:10.1016/S0341-8162(01)00164-3.

Reichenzeller, E., et al. (2026). Individual tree stem detection using the Vertical Complexity Index on structure-from-motion point clouds. *Forests* 17(7), 807. doi:10.3390/f17070807.

Theinat, A.K., Modiriasari, A., Bobet, A., Melosh, J., Dyke, S., Ramirez, J., Maghareh, A., & Gomez, D. (2018). Geometry and structural stability of lunar lava tubes. *2018 AIAA SPACE and Astronautics Forum and Exposition*, paper 2018-5185. doi:10.2514/6.2018-5185.

Wagner, R.V. & Robinson, M.S. (2021). Occurrence and origin of lunar pits: observations from a new catalog. *LPSC 52*, Abstract #2530 (LPI Contrib. No. 2548).
...
Wong, U., Whittaker, W., Jones, H., & Whittaker, R. (2014). NASA Planetary Pits and Caves Analog Dataset [data set]. NASA Ames Research Center. https://ti.arc.nasa.gov/dataset/caves
