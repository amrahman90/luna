...
Script — invoke as ``python explore_indian_tunnel.py``; import-safe
(no side effects on import). All file/matplotlib side effects are
gated behind ``main()``.
...
def quick_dtm(x, y, z, spacing=0.5):
    """Build a regular-grid DTM at the given spacing (mean of in-cell z)."""
...
def main() -> None:
    """Load three Indian Tunnel clouds, render preflight PNG, frame-check.

    Side effects: writes ``OUT / "preflight_clouds.png"`` (PROVENANCE_INDEX
    row 47, status=canonical) and prints per-site stats + frame-consistency
    stats to stdout.
    """
