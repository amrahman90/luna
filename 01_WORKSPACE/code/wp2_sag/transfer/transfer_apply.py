...
def find_score_path(dtm_name: str, rung: float) -> Path | None:
    """Locate the cached score raster across all per-DTM subdirs.

    Search paths (first hit wins):
      1. ~/lunarvoid/data/outputs/wp2_sag/score_rasters/<DTM>/score_<rung>m.tif
         (P3.1c growth: new rasters generated by score_raster_gen.py live here
         per conventions §1 — derived rasters under ~/lunarvoid/data/)
      2. 01_WORKSPACE/data/outputs/wp2_sag/<sub>/<DTM>_<rung>m_score.tif
         (legacy location used by the 7 P3.1b cached sites; preserved for
         byte-identity with the N=19 re-run)
    """
    # New (P3.1c growth): ~/lunarvoid/data/outputs/wp2_sag/score_rasters/<DTM>/
    new_dir = RAW / "outputs" / "wp2_sag" / "score_rasters" / dtm_name
    new_cand = new_dir / f"score_{rung:g}m.tif"
    if new_cand.exists():
        return new_cand
    # Legacy (P3.1b frozen-cache): 01_WORKSPACE/data/outputs/wp2_sag/<sub>/
...
def discover_dtms_and_rungs() -> list[tuple[str, float]]:
    """Walk both score-raster roots and return sorted unique (DTM, rung)
    pairs.

    Roots (concatenated, then dedup'd by (DTM, rung) — first hit wins):
      1. ~/lunarvoid/data/outputs/wp2_sag/score_rasters/<DTM>/score_<rung>m.tif
         (P3.1c growth: new rasters generated by score_raster_gen.py live here
         per conventions §1 — derived rasters under ~/lunarvoid/data/).
         Filename pattern: score_<rung>m.tif
      2. 01_WORKSPACE/data/outputs/wp2_sag/<sub>/<DTM>_<rung>m_score.tif
         (legacy P3.1b frozen cache).
         Filename pattern: <DTM>_<rung>m_score.tif
    """
    out = {}
    # New root: ~/lunarvoid/data/outputs/wp2_sag/score_rasters/
    new_root = RAW / "outputs" / "wp2_sag" / "score_rasters"
    if new_root.exists():
        for d in new_root.iterdir():
            if not d.is_dir():
                continue
            for f in d.glob("score_*.tif"):
                m = re.match(r"score_(\d+(?:\.\d+)?)m\.tif", f.name)
                if m:
                    out[(d.name, float(m.group(1)))] = f
    # Legacy root: 01_WORKSPACE/data/outputs/wp2_sag/<sub>/
    for sub in WP2.iterdir():
        if not sub.is_dir():
            continue
...
def transfer_dtm_rung(dtm: str, rung: float, floors_row, rille_gdf,
                      registry_lines: list, existing_ids: set = None,
    ...
    # P3.1c growth (highland extrapolation): if local_Amin is NaN, the
    # DTM has no usable flat panels (highland/impact-melt terrain),
    # so we cannot say a peak is above the local detection floor.
    # Mark such peaks as below-floor by default; this prevents FPs
    # from being counted at sites where the FROZEN TRANQPIT1
    # calibration is extrapolation (highland), not portability.
    if math.isnan(local_Amin):
        no_local_floor = True
    else:
        no_local_floor = False
    ...
    for rank, p in enumerate(peak_records, start=1):
        ...
        amp = p["depth_proxy_m"]
        # below-floor: NaN local_Amin -> always below-floor (terrain
        # extrapolation, no local floor available, conservative: do not
        # count FPs at sites where the FROZEN calibration is extrapolation)
        if no_local_floor:
            below_floor = True
        else:
            below_floor = amp < local_Amin
    ...
    n_dup = 0
    for rec in registry_local:
        # NEW (P3.1c N=19): skip if candidate_id already exists in registry
        if existing_ids is not None and rec["candidate_id"] in existing_ids:
            n_dup += 1
            continue
        notes = "below-local-floor" if rec["below_local_floor"] else ""
        if no_local_floor:
            notes = (notes + "; " if notes else "") + "terrain-extrapolation (no local_Amin; FROZEN TRANQPIT1 is mare-only; highland/impact-melt inference)"
        if rec["tier"] == "B":
    ...
    return {
        ...
        "registry_new_appends": len(registry_local) - n_dup,
        "registry_dups_skipped": n_dup,
    }
...
def main():
    ...
    # NEW (P3.1c N=19): also emit per-DTM entries for on-disk DTMs that
    # have NO cached v0.1 score raster. They contribute 0 candidates and
    # 0 area but ARE part of the N=19 honesty banner. This is the only
    # change in transfer_apply.py for the N=19 re-run.
    score_dtm_set = set(p[0] for p in pairs)
    skipped_no_score = sorted(set(floors.keys()) - score_dtm_set)
    print(f"[init] floors table DTMs: {len(floors)}; with score raster: "
          f"{len(score_dtm_set)}; no score raster: {len(skipped_no_score)} "
          f"({skipped_no_score})", flush=True)
    pair_results = []  # initialise BEFORE stub append
    # register stub pair_results for the skipped DTMs
    for dtm in skipped_no_score:
        pair_results.append({
            "dtm": dtm, "rung_m": None, "error": "no cached score raster",
            "n_candidates": 0, "n_above_local_floor": 0,
            "n_below_local_floor": 0, "n_tier_B": 0,
            "n_fp": 0, "n_tp": 0, "area_km2": 0.0,
            "from_cache": False, "n_pits_in_dtm": 0,
            "registry_count": 0,
            "top_score": 0.0,
            "local_3sigma_m": float(floors[dtm]["three_sigma_m"]) if dtm in floors else None,
            "local_Amin_m": float(floors[dtm]["local_Amin_m"]) if dtm in floors else None,
        })
    ...
    # NEW (P3.1c N=19): build a set of existing candidate_ids so we skip
    # duplicates on re-run. The frozen-frac/slope/neigh recipe is
    # deterministic per (DTM, rung), so re-running produces the SAME rows.
    import re as _re
    existing_ids = set()
    for line in reg_lines:
        if line.startswith("#") or line.startswith("candidate_id"):
            continue
        parts = line.split(",")
        if parts and parts[0].startswith("LV-"):
            existing_ids.add(parts[0])
    print(f"[init] existing candidate_ids in registry: {len(existing_ids)}",
          flush=True)
    for dtm, rung in pairs:
        floors_row = floors.get(dtm)
        if floors_row is None:
            # NEW (P3.1c growth): if no floors entry (e.g. FRESHMELT/FRESHMELT1
            # which were panel-skipped at P3.1a), synthesise a NaN row so the
            # score raster is processed. With NaN local_Amin, the below-floor
            # rule fires and no FPs are counted — peaks are kept in the
            # registry with "below-local-floor; terrain-extrapolation".
            floors_row = pd.Series({
                "dtm_name": dtm, "site": dtm, "lon_min": np.nan, "lon_max": np.nan,
                "lat_min": np.nan, "lat_max": np.nan, "n_panels": 0,
                "pooled_rms_m": np.nan, "three_sigma_m": np.nan,
                "panel_min_rms": np.nan, "panel_max_rms": np.nan,
                "local_Amin_m": np.nan, "mtime": "skipped_insufficient_panels",
            })
            print(f"[warn] {dtm}: no entry in per_dtm_floors.csv; processing "
                  f"with NaN local_Amin (terrain extrapolation)", flush=True)
        t1 = time.time()
    ...
    # per-rung aggregate (skip stub entries with rung_m=None)
    per_rung = {}
    for r in pair_results:
    ...
    # N=19/649 honesty: count of DTM sites that have NAC DTM on disk (P3.1c)
    n_10 = len(floors)  # all DTMs in the floors table (P3.1a outputs)
    n_score_raster = len(set(p[0] for p in pairs))
    n_no_raster = skipped_no_score  # use the pre-computed list (P3.1c)

    agg_pt, agg_lo, agg_hi = poisson_fp_ci(total_fp, total_area)

    summary = {
        "generated": date.today().isoformat(),
        "scope_banner": f"N={n_10}/649 ({n_10} good-tier DTMs on disk of 649 in scope; "
        ...
        "aggregate": {
            ...
            # P3.1c skeptic review (2026-08-22): framing fields
            "fp_per_1e4km2_interpretation": (
                "calibration-context rate, selection-biased to catalogued pits "
                f"(all {n_10} DTMs on disk are pit-associated or pit-rich); "
                "NOT a random-mare survey rate"
            ),
            "highland_extrapolation_note": (
                "5 highland-composition sites (KINGCRATER2/3/4 central peak, "
                "TYCHOPK02/03/04/07 Tycho central peak) plus 2 impact-melt sites "
                "(FRESHMELT, FRESHMELT1) have NaN local_Amin (no usable flat panels); "
                "the FROZEN TRANQPIT1 calibration is mare-only. Their peaks are "
                "marked below-local-floor by default and DO NOT contribute to FP "
                "counting — but ARE preserved in the registry with the "
                "terrain-extrapolation annotation. Any morphometric inference at "
                "these sites with the frozen thresholds is extrapolation, not "
                "portability."
            ),
            "scope_caveats": [
                f"all {n_10} on-disk DTMs are pit-associated or impact-melt "
                "(selection bias toward catalogued pits)",
                "7 highland/impact-melt sites have NaN local_Amin and are "
                "treated as terrain extrapolation, not portability",
                "no random-mare control; rate is calibration context only",
                "TYCHOPK (1.44 GiB float32) skipped due to memory ceiling; "
                "deferred to post-G2 (needs Tier-1 rental or memory-efficient "
                "tile-based processing)",
                "frozen TRANQPIT1 recipe unchanged from P3.1b freeze",
            ],
        },
